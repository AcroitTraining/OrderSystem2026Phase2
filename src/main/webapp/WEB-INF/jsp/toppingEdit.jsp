...
<body>
    ...
    <div class="main-card">
        <form action="ToppingEditServlet" method="post" id="toppingForm">
            ...
            <div class="form-group">
                ...
                <input type="text" name="toppingName" id="toppingName" class="input-topping-name" value="<%= info.getToppingName() %>">
            </div>

            <div class="form-group">
                <label class="label-title">金額 <span class="label-sub">※最大5桁</span></label>
                <input type="number" name="toppingPrice" id="toppingPrice" class="input-price" value="<%= isNew ? "" : info.getToppingPrice() %>" placeholder="0">
                <span class="currency-unit">円</span>
            </div>

            <div class="footer-actions">
                <a href="ToppingListServlet" class="btn-back">トッピング一覧へ戻る</a>
                <button type="button" class="btn-submit" id="openModalBtn"><%= isNew ? "作成" : "変更" %></button>
            </div>
        </form>
    </div>
    ...
    <div class="modal-overlay" id="modalOverlay" style="display:none;">
        <div class="modal-content">
            <h3><%= isNew ? "この内容で作成しますか？" : "この内容で変更しますか？" %></h3>
            <div class="modal-body">
                <div class="modal-row">
                    <span class="modal-label">トッピング名</span>
                    <span class="modal-val" id="modalToppingName"></span>
                </div>
                <div class="modal-row">
                    <span class="modal-label">金額</span>
                    <span class="modal-val" id="modalToppingPrice"></span>
                </div>
            </div>
            <div class="modal-actions">
                <button type="button" class="btn-modal-no" id="modalNoBtn">いいえ</button>
                <button type="button" class="btn-modal-yes" id="modalYesBtn"><%= isNew ? "作成する" : "変更する" %></button>
            </div>
        </div>
    </div>
