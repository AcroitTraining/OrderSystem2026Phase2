...
public class ToppingEditServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try (Connection conn = DBConnectionUtil.getConnection()) {
			...
			ToppingEditInfo info = dao.findToppingDetails(conn, toppingId);
			if (info != null) {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try (Connection conn = DBConnectionUtil.getConnection()) {
			if (toppingIdStr == null || toppingIdStr.isEmpty() || "0".equals(toppingIdStr)) {
				// 新規追加
				dao.insertToppingDetails(conn, toppingName, toppingPrice);
			} else {
				// 更新
				int toppingId = Integer.parseInt(toppingIdStr);
				dao.updateToppingDetails(conn, toppingId, toppingName, toppingPrice);
			}
			response.sendRedirect("ToppingListServlet");
		} catch (SQLException e) {
			e.printStackTrace();
